...
pinMode(LED_PIN, OUTPUT);
  ...
  delay(1000);
  Serial.println("READY"); // Segnale fondamentale per lo script Python
}
...
if (c == '\n' || c == '\r') {
      if (bufferIndex > 0) {
        ...
        // Se c'è un prefisso, saltalo per i controlli (4 caratteri)
        if (isAsm || isHex) {
...
int res2 = controllo_dinamico_loop(dataStart, dataStart + dataLen);
          ...
          if (res2 == 0) {
            ...
            // Calcola res3 SOLO se è stato caricato un file Assembly
            if (isAsm) {
                Serial.println("Esecuzione riscvprg...");
...
delay(res3.tempo);
                ...
                Serial.println("Modalita' esadecimale: res3 non calcolato.");
            }
          }
          else {
